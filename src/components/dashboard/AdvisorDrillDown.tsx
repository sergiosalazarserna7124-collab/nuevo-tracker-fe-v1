...
function LeadDetalleCrossCanal({ lead, data, onBack }: { lead: AsesorLeadCRM; data: AsesorResponse; onBack: () => void }) {
  ...
  const interacciones = useMemo(() => {
    ...
    for (const c of data.chats.filter((c) => matchLead(c.leadEmail, c.leadName, null))) {
      ...
      out.push({ canal: "chat", date: c.fechaUltimoMensaje, label: c.estado ?? (c.respondido ? "Respondido" : "Sin respuesta"), texto: ultimo ? `${ultimo.role === "lead" ? "Lead" : "Asesor"}: ${ultimo.message}` : `${c.messages.length} mensajes` });
    }
    for (const v of data.videollamadas.filter((v) => matchLead(v.leadEmail, v.leadName, v.ghlContactId))) {
      out.push({ canal: "cita", date: v.fechaReunion, label: categoriaLabel(v.categoria), texto: v.resumenIa ?? "Sin resumen IA", categoria: v.categoria });
    }
      ...
      {filtradas.length === 0 ? (
        ...
        <div className="space-y-1.5 flex-1 overflow-y-auto">
          {filtradas.map((i, idx) => {
            ...
            return (
              <div key={idx} className="rounded-lg bg-surface-700 px-3 py-2">
