...
function LeadDetalleCrossCanal({ lead, data, onBack }: { lead: AsesorLeadCRM; data: AsesorResponse; onBack: () => void }) {
  ...
  // Conteo de llamadas: total y cuántas contestaron (para el registro resumen del listado).
  const llamadaCounts = useMemo(() => {
    const ll = interacciones.filter((i) => i.canal === "llamada");
    const contestadas = ll.filter((i) => i.nota && esLlamadaEfectiva(i.nota)).length;
    return { total: ll.length, contestadas };
  }, [interacciones]);
  ...
  useEffect(() => {
    if (!contextoIA.trim()) { setResumenLead(null); setResumenLoading(false); setResumenErr(null); return; }
    let cancelled = false;
    ...
    fetch("/api/data/lead-resumen", {
      ...
      .catch((e: unknown) => { if (!cancelled) setResumenErr(e instanceof Error ? e.message : "Error"); })
  ...
  const filtradas = interacciones.filter((i) => {
    ...
    // Ocultar llamadas NO contestadas del listado (ruido "Sin análisis"); van resumidas
    // en el registro de conteo. Excepción: si el usuario filtra explícitamente por su sub-categoría.
    if (i.canal === "llamada" && i.nota && !esLlamadaEfectiva(i.nota) && subCat === "todas") return false;
    return true;
  });
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* Resumen general del lead (IA, cruzando todos los canales): último acuerdo / tarea pendiente */}
      {interacciones.length > 0 && (
        <div className="rounded-lg border border-accent-purple/30 bg-accent-purple/5 p-3 mb-3">
          <h4 className="text-xs font-semibold text-accent-purple flex items-center gap-1 mb-1">
            ...
            <span className="text-[9px] font-normal text-gray-500">· último acuerdo · todos los canales</span>
          </h4>
          {resumenLoading ? (
            ...
            <p className="text-xs text-amber-400/90">No se pudo generar el resumen{resumenErr ? `: ${resumenErr}` : "."}</p>
          )}
        </div>
      )}
      ...
      {/* Registro resumen de llamadas: total y contestadas (las no contestadas no se listan) */}
      {llamadaCounts.total > 0 && (canal === "todas" || canal === "llamada") && subCat === "todas" && (
        <div className="flex items-center gap-2 rounded-lg bg-surface-700/60 border border-surface-500 px-3 py-2 mb-1.5">
          <Phone className="w-3.5 h-3.5 text-accent-cyan shrink-0" />
          <span className="text-xs text-gray-300">
            <span className="font-semibold text-white">{llamadaCounts.total}</span> llamada{llamadaCounts.total !== 1 ? "s" : ""} ·{" "}
            <span className="font-semibold text-accent-cyan">{llamadaCounts.contestadas}</span> contestada{llamadaCounts.contestadas !== 1 ? "s" : ""}
          </span>
          <span className="text-[10px] text-gray-500 ml-auto">las no contestadas no se listan</span>
        </div>
      )}
